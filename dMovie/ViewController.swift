...
//  dMovie
//
//  Created by David Lee on 2/20/21.
//
...
//   print("self.movies.count is \(self.movies.count) before url request")
   ...
   let task = session.dataTask(with: request) { [self] (data, response, error) in
      ...
      if let error = error {
       ...
       print("self.movies.count is \(self.movies.count) after url request")
       
       // This below statement is to reload tableView because tableView function is one time right after launching the initial view.
